...
NOTES_FILE="$PLEAC_DIR/slup-notes.txt"
TMP_DIR="/tmp/pleac-slup-pass.$$"
...
while IFS= read -r section_dir; do
  ...
  slup_file="$section_dir/slup.slup"

  ref_out="$TMP_DIR/ref.out"
  ref_err="$TMP_DIR/ref.err"
  ref_out_norm="$TMP_DIR/ref.out.norm"
  ref_err_norm="$TMP_DIR/ref.err.norm"
  slup_out="$TMP_DIR/slup.out"
  slup_err="$TMP_DIR/slup.err"
  slup_out_norm="$TMP_DIR/slup.out.norm"
  slup_err_norm="$TMP_DIR/slup.err.norm"
...
EOF
  ...
  run_with_timeout 3 "$slup_out" "$slup_err" perl "$ROOT/slup.pl" "$slup_file"
  slup_status=$?
  if [ "$slup_status" -ne 0 ]; then
    append_note "$section_rel: slup wrapper failed (status=$slup_status, baseline=$ref_lang)"
    sections_noted=$((sections_noted + 1))
    continue
  fi
  ...
  normalize_stream "$slup_out" "$slup_out_norm"
  normalize_stream "$slup_err" "$slup_err_norm"

  if cmp -s "$ref_out_norm" "$slup_out_norm" && cmp -s "$ref_err_norm" "$slup_err_norm"; then
